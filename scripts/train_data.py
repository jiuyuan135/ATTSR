...
if __name__ == '__main__':
    ...
    x_data = sorted(
        glob.glob(os.path.join(r"D:\tyb\RS\data\Austria\train\img", '*.tif')))
    y_data = sorted(
        glob.glob(os.path.join(r"D:\tyb\RS\data\Austria\train\dem", '*.tif')))

    dataset = [x_data, y_data]
    train_loader, __ = build_dataloader(dataset, dataset, int(batch_size))

    model_save_path = r"D:\tyb\RS\checkpoints\BIF.pth"
    loss_save_path = r"D:\tyb\RS\checkpoints\BIF.txt"
